...
export default function PorscheAIDashboard() {
  ...
  return (
    ...
    >
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={selected.history.map((v, i) => ({ i, v }))}>
                  ...
                  <Tooltip
                    ...
                    labelFormatter={() => ""}
                  />
                  <Line type="monotone" dataKey="v" stroke="#C9A968" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>

            <p
              className="porsche-font-body text-[11px] uppercase tracking-wide mb-2"
              style={{ color: "#6f6f74" }}
            >
              Últimos insights
            </p>
            <div className="flex flex-col gap-2 max-h-40 overflow-y-auto feed-scroll">
              {selectedFeed.length === 0 && (
                <p className="porsche-font-body text-xs" style={{ color: "#6f6f74" }}>
                  Nenhum insight recente para este agente.
                </p>
              )}
              {selectedFeed.map((f) => (
                <div key={f.id} className="flex gap-2">
                  <span className="porsche-font-mono text-[10px] mt-0.5 shrink-0" style={{ color: "#6f6f74" }}>
                    {f.time}
                  </span>
                  <p className="porsche-font-body text-xs" style={{ color: "#C7C6C2" }}>
                    {f.text}
                  </p>
                </div>
              ))}
      ...
      <footer
        className="px-6 sm:px-10 py-4 porsche-font-body text-[11px] text-center"
        style={{ color: "#54545a", borderTop: "1px solid #1F1F22" }}
      >
        Dados simulados para fins de demonstração — não representam operações reais da Porsche.
      </footer>
    </div>
  );
}
